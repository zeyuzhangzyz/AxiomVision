...
class DetInferencer(BaseInferencer):
    ...
    def __call__(
        ...
        self.conf_score_thr = conf_score_thr
        ...
        with open("mmdetection_time.txt", "a+") as file:
                file.write(f"{total_time}, {inputs}, mmdetection\n")
        return results_dict
    ...
    def pred2dict(self,
        ...
        # zeyu 2025/5/22 add score threshold

        result = {}
        if 'pred_instances' in data_sample:
            masks = data_sample.pred_instances.get('masks')
            pred_instances = data_sample.pred_instances.numpy()
            valid_inds = pred_instances.scores > self.conf_score_thr
            
            result = {
                'labels': pred_instances.labels[valid_inds].tolist(),
                'scores': pred_instances.scores[valid_inds].tolist()
            }
            
            if 'bboxes' in pred_instances:
                result['bboxes'] = pred_instances.bboxes[valid_inds].tolist()
            ...
            if masks is not None:
                ...
                encode_masks = encode_mask_results(pred_instances.masks[valid_inds])
