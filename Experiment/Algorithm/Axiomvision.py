...
class Edge_server:
    ...
    def update(self, camera_index, t):
        ...
        if reconnect == True:
            print('reconnect')
            reconnect = False
            self.G = nx.generators.classic.complete_graph(self.camera_index_list)
            new_group_cameras = dict()
            camera_index = []
            for k in list(self.G.nodes()):
                camera_index.append(k)
                new_group_cameras[k] = self.groups[self.locate_camera_index(k)].cameras[k]
            for i in self.camera_index_list:
                self.group_inds[i] = 0
            c = 0
            self.groups = {}
            self.groups[c] = Base.Group(b=sum([new_group_cameras[n].b for n in new_group_cameras]),
                                            ...
                                            camera_num=len(new_group_cameras),
                                            rounds=self.rounds, cameras=copy.deepcopy(new_group_cameras),
                                            payoffs=sum([new_group_cameras[k].payoffs for k in new_group_cameras]),
                                            best_payoffs=sum([new_group_cameras[k].best_payoffs for k in new_group_cameras]))
            self.num_groups[t + 1] = 1
...
class Cloud_server:
    ...
    def run(self, envir, T):
        ...
        # with open(f"time{self.txt_name}_Axiomvision_{T}.txt", "w") as file:
        for i in range(1, T + 1):
            if i % 5000 == 0:
                print(i, len(edge_server.groups))
            if np.random.random() < prob / i**2 and i>10:
                reconnect = True
            # if i % 1000 == 0 :
            ...
            # through the last element to decide the current model level to choose three different levels, this parameter also work in select and feedback function
            if last_element == 0:
